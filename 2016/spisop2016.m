function [ out ] = spisop2016(functionName,currentFullInstallationFilePath,inputFolderName,outputFolderName,coreParamFile,functionParamFile,varargin)
...
[pathInputFolder, pathOutputFolder] = spisop_init_helper2016(pathPrefix,inputFolderName,outputFolderName,parallelComputation,numParallelWorkers);
try
    if (~isdeployed)
...
switch lower(functionName) 
    ...
    case 'browser'
        fprintf(['browser function with not supported for the SpiSOP based on Matlab 2016'])
        printUsage();
        if (~isdeployed)
            error('try again!');
        else
