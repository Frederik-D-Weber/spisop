function convert_channelwise_ascii(currentFullInstallationFilePath,fileFilterSettings,fileRerefSettings,DoReReference,linearDeviationMontagePath,ApplyLinearDeviationMontage)
...
for iData = 1:(length(dataFilesNames))
    ...
    for iSplitFactor = 1:length(splitFactors)
        ...
        curr_channelColumns_raw = file_chanelColumns(curr_splitFactorsRows(curr_splitFactorsRows_raw_index));
...
%         hdr.brainvision_outformat = 'float32';%float32 int16 int32;
%         data_file_name_current = [data_file_name_current '_' 'filtered' '_' hdr.brainvision_outformat];
%         ft_write_data([data_file_name_current '.eeg' ], data.trial{:},'dataformat',dataformat,'header',hdr);
%         
        dataformat = 'ascii';
        
        data_file_name_current = [data_file_name_current '_' 'filtered' '_' dataformat '_' num2str(data.fsample) 'Hz'];
        
        f_export = fopen([data_file_name_current '.csv'],'wt','n','US-ASCII');
        
        data_export_format_string = '';
        for iLabel = 1:numel(hdr.label)
            if iLabel ~= numel(hdr.label)
            	fprintf(f_export,'%s,',hdr.label{iLabel});
                data_export_format_string = [data_export_format_string '%.4f,'];
            else
            	fprintf(f_export,'%s\n',hdr.label{iLabel});
                data_export_format_string = [data_export_format_string '%.4f\n'];
            end
        ...
        for iRow = 1:size(dat,1)
            fprintf(f_export,data_export_format_string,dat(iRow,:));
        end
        dat = [];
        
        %writetable(array2table(','VariableNames',hdr.label),[data_file_name_current '.dat'],'Delimiter',',');
