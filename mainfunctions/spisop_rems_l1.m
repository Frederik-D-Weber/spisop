function [res_filters, res_channels, res_events] = spisop_rems_l1(pathInputFolder, pathOutputFolder, ouputFilesPrefixString, listOfCoreParameters, listOfParameters)
...
tic
memtic
fprintf('REMs function initialized\n');
...
%     end
    ...
    %%%low pass filter the EOGs
    cfg = [];
    cfg = core_cfg;
    cfg.lpfilter = 'yes';
    FpassRight = EOG_low_pass_filter_freq; %right pass frequency in Hz
    ...
    data_lpfilt = ft_fw_preprocessing(cfg,data);
    
    
    
    %%%additional high pass filter the EOGs
    cfg = [];
    cfg = core_cfg;
    cfg.hpfilter = 'yes';
    FpassLeft = EOG_high_pass_filter_freq; %left pass frequency in Hz
    ...
    data_lphpfilt = ft_fw_preprocessing(cfg,data_lpfilt);
    
    
    
    data_lpfilt_dev = apply_deviation_per_second_to_data(data_lpfilt);
    data_lpfilt = [];
    data_lphpfilt_dev = apply_deviation_per_second_to_data(data_lphpfilt);
    data_lphpfilt = [];
    
    cfg = []
    cfg.feedback = core_cfg.feedback;
    cfg.channel = channel_label_EOG_left;
    data_lpfilt_dev_EOG_left = ft_selectdata(cfg,data_lpfilt_dev);
    data_lphpfilt_dev_EOG_left = ft_selectdata(cfg,data_lphpfilt_dev);
    
   
    cfg.channel = channel_label_EOG_right;
    data_lpfilt_dev_EOG_right = ft_selectdata(cfg,data_lpfilt_dev);
    data_lphpfilt_dev_EOG_right = ft_selectdata(cfg,data_lphpfilt_dev);
...
%     minFreqPostFreqBorderBufferLength = round(FrqOfSmpl/minFreq);
    ...
    %for iChan = 1:nChannels
        ...
        for iTr = 1:numel(data_lpfilt_dev_EOG_right.trial)
                     
            %iTr = 4;
            fprintf('dataset %i: subpart %i, preselect \n',iData,iTr);
            rawDataSampleOffset = trlSampleBeginsAndEnds(iTr,1) - 1;
            
            
            signal_lpfilt_dev_EOG_right = data_lpfilt_dev_EOG_right.trial{iTr};
            signal_lpfilt_dev_EOG_left = data_lpfilt_dev_EOG_left.trial{iTr};
            
            signal_lphpfilt_dev_EOG_right = data_lphpfilt_dev_EOG_right.trial{iTr};
            signal_lphpfilt_dev_EOG_left = data_lphpfilt_dev_EOG_left.trial{iTr};


                  
             basic_deviation_per_second_threshold; %?V/s 
    relaxed_deviation_per_second_threshold; %?V/s
    
    EOGleft_candidate_idx_first = (signal_lpfilt_dev_EOG_left > basic_deviation_per_second_threshold) & ...
        (signal_lphpfilt_dev_EOG_left > basic_deviation_per_second_threshold) & ...
        (signal_lphpfilt_dev_EOG_right < (basic_deviation_per_second_threshold - relaxed_deviation_per_second_threshold);
    
    EOGleft_candidate_idx_second = (signal_lpfilt_dev_EOG_left > relaxed_deviation_per_second_threshold) & ...
        (signal_lphpfilt_dev_EOG_left > relaxed_deviation_per_second_threshold) & ...
        (signal_lphpfilt_dev_EOG_right < 0) & ...
        
........    
    


    EOGleft_candidate_idx = EOGleft_candidate_idx_first | EOGleft_candidate_idx_second
        
    
    Y1 = diff(LowPassFltrdEog1);%derivative
Y2 = diff(LowPassFltrdEog2);%derivative

Y1Filtered = diff(eog1Filtered);%derivative
Y2Filtered = diff(eog2Filtered);%derivative
...
eogNoise1 = determineSignalNoise( originalEog1, sampling, eogClass );
eogNoise2 = determineSignalNoise( originalEog2, sampling, eogClass );
...
thrBS = 0.7*eogNoise; %basic steepness threshold
thrRS = 0.66*thrBS; % relaxed steepness threshold, 0.462
...
for i = 1:gap:sigLen - gap % -> for each epoch
            ...
            (1)	If DRleft(i) > bT and filtDRleft(i) > bT and DRright(i) < bT - rT
	     EOGleft(i) valid candidate point
(2)	Elseif DRleft(i) > rT and filtDRleft(i) > rT and DRright(i) < 0 and DRleft(i) > 0 for >= 100 ms and EOGleft(i) > -100 ?V
	     EOGleft(i) valid candidate point
(3)	Else
...
fprintf('REMs function finished\n');
toc
memtoc
end
...
%%% SUBFUNCTION
%%%%%%%%%%%%%%%%
function data = apply_deviation_per_second_to_data(data)
for iTr = 1:numel(data.trial)
    d = diff(data.trial{iTr},1,2);
    d = d ./ data.fsample;
    d = [zeros(size(d,1),1) diff(data.trial{iTr},1,2)];
    data.trial{iTr} = d;
end
end
